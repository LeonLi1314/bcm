	...
	private void init() {
		synchronized (this) {
			try {
				...
				// menuid = menu.value();
				//
				// PerMenu sysMenu = PerMenu.INSTANCE.queryOne("menuname=?",
				// menu.value());
				//
				// if (sysMenu == null) {
				...
				// "menuid = ? and action=?", m.getMenuid(),
				// m.getAction());
				// if (ma == null) {
				// m.insert();
				// }
				// methodActionMap.put(m1.getName(), m.getAction());
				// }
				// }
				// DBManager.commitAll();
...
//		PerUser user = PerUser.INSTANCE.load(iuser.getUserId());
