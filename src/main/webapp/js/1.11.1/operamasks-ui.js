	...
	_renderProxy: function() {
		...
		if(this._helper) {
			...
			this.helper.addClass(this._helper).css({
				width: this.element.outerWidth(true) + pxyoffset,
				height: this.element.outerHeight(true) + pxyoffset,
				position: 'absolute',
...
$.om.plugin.add("omResizable", "containment", {
	...
	stop: function(event, ui){
		...
		var helper = $(self.helper), ho = helper.offset(), w = helper.outerWidth(true) - self.sizeDiff.width, h = helper.outerHeight(true) - self.sizeDiff.height;
...
$.omWidget("om.sortable", $.om.omMouse, {
	...
	refreshPositions: function(fast) {
		...
		for (var i = this.items.length - 1; i >= 0; i--){
			...
			if (!fast) {
				item.width = t.outerWidth(true);
				item.height = t.outerHeight(true);
			}
...
(function($) {
	...
	$.omWidget("om.omPanel" , {
		...
		_init: function(){
			...
			this._bindEvent();
		 	this._resize($parent);
		 	var headerHeight = options.header !== false? $header.outerHeight(true) : 0;
		 	if(options.collapsed !== false){
		 		...
		 		$body.show();
		 		"auto"!==options.height && $parent.height(headerHeight + $body.outerHeight(true));
		 		if(options.header !== false){
		 ...
		 */
	 	_resize: function($panel){
	 		...
	 		if(options.width == 'fit'){
				...
				$body.outerWidth($panel.width());
	 			...
	 			if(style && style.indexOf("width") !== -1){
	 				$panel.width($body.outerWidth(true));
	 				$header.outerWidth($body.outerWidth(true));
	 			}else{
	 				$panel.css("width" , "");
		 			$header.css("width" , "");
		 			$body.css("width" , "");
	 		...
	 		if(options.height == 'fit'){
	 			options.height = '100%';
	 			$panel.height('100%');
	 			$body.outerHeight($panel.height()- (this.options.header!==false?$header.outerHeight(true):0) );	 
				...
				$body.outerHeight($panel.height()- (this.options.header!==false?$header.outerHeight(true):0) );	 
	 			...
	 			if(style && style.indexOf("height") !== -1){
	 				$panel.height($header.outerHeight(true) + $body.outerHeight(true));
	 			}else{
	 				$panel.css("height" , "");
		 			$body.css("height" , "");
	 			}
		...
		expand: function(/**anim , speed**/){
			...
			"auto"===options.height && $parent.height($header.outerHeight(true));
			$body.show();
			if($body.data("loading")){
				$loadMessage.show();
			}
...
$.omWidget( "om.omAccordion", {
    ...
    disable : function() {
        ...
        if( ($disableDiv = $acc.find(">.om-accordion-disable")).length === 0 ){
	        $("<div class='om-accordion-disable'></div>").css({position:"absolute",top:0,left:0})
	        	.width($acc.outerWidth(true)).height($acc.outerHeight(true)).appendTo($acc);
        }
        $disableDiv.show();
    },
    /**
    ...
    resize: function() {
    	...
    	$.each(panels , function(index , panel){
    		headerHeight += panel.prev().outerHeight(true);
    	});
    	$.each(panels , function(index , panel){
	...
	$.omWidget('om.omCombo', {
        ...
        _init:function(){
            ...
            if (options.width != 'auto') {
                ...
                inputEl.width(span.innerWidth() - inputEl.next().outerWidth(true) - inputEl.outerWidth(true) + inputEl.width());
            }
        ...
        _showDropList:function(){
            ...
            if (!options.listAutoWidth) {
                dropListContainer.width(span.outerWidth(true));
            }else{
            	if($.browser.msie&&($.browser.version == "7.0")&&!$.support.style){
            		dropListContainer.width(dropList.show().outerWidth(true));
            	}else{
            		dropListContainer.width(dropList.outerWidth(true));
            	}
            }
    	...
    	self._getHeaderCols().each(function(index){
        	...
        	if(axis.substring(0,3)=='col'){
        		...
        		if($.isEmptyObject(self._editComps)){//保证再次编辑其它行时不会重复添加 padding-left.
        			$editRow.css("padding-left", parseInt($editRow.css("padding-left")) + $cell.outerWidth(true));
        		}
        		return ;
        	}
        	...
        	editComp = self._editComps[compKey];
   			...
   			if(!$ins){
   				...
   				if("omCalendar"==type || "omCombo"==type){
   					...
   					if("omCalendar"==type){
   						$ins.val(lastValue).width($cell.outerWidth(true)-24);
...
//			$editRow,
	...
	function _resizeView(self , $col , differWidth){
		...
		$editView.width($elem.outerWidth(true));
		self._getHeaderCols().each(function(index , th){
			...
			$.each(self._editComps , function(name , comp){
				...
				if(id == comp.id){
					...
					if(!target){
						$ins.closest("div.grid-edit-wrapper").width($th.outerWidth(true)).css("left" , $th.position().left + scrollLeft);
					}
					//改变编辑输入域组件的宽度
					if("omCalendar"==type || "omCombo"==type){
						var $parent = $ins.parent();
						if("omCalendar"==type){
	   						$ins.width($th.outerWidth(true)-24);
...
//			$editRow = self._editView.editRow,
	...
	function posErrorHolder(self , $ins , errorHolder){
		var docPos = $ins.offset(),
		    ...
		    top = docPos.top-tablePos.top+$elem.outerHeight(true);
		    //把错误定位到左边
			if(docPos.left-tablePos.left-$bDiv.scrollLeft() + $elem.outerWidth(true) + errorHolder.outerWidth(true) > $bDiv.width()){
				errorHolder.css({left:docPos.left-tablePos.left-errorHolder.outerWidth(true),top:top});
			}else{
				errorHolder.css({left:docPos.left-tablePos.left+$elem.outerWidth(true),top:top});
			}
	}
})(jQuery);
/*
...
(function($) {
    ...
    function _resizeRowExpander(){
    	...
    	var width = $ht.width() - $ht.find("th.expenderCol").outerWidth(true) - parseInt($rowExpander.find("> td").css("border-right-width"));
    ...
    $.omWidget('om.omItemSelector', {
        ...
        _resizePanel:function(){
            var self = this, lp=self.leftPanel,
                ...
                W=($('tr',self.element).innerWidth()-self.toolbar.outerWidth(true))/2,
                innerW=lp.width(W).innerWidth();
        ...
        _buildList:function(){
            ...
            var items = $('.om-itemselector-items'),itemdtH =$('>dl>dt',items).outerHeight(true),
            h =items.outerHeight(true),offset = itemdtH - h%itemdtH;
        ...
        _page: function(btn, isup){
        	...
        	$dl = $items.children("dl"), h=$items.outerHeight(true)-2,dlH = $dl.outerHeight(true),
        	dltop =$dl.offset().top, nextbtn, top = this.element.data("dltop")+20;
        	if(isup){
        		$dl.offset({top: dltop+h});
        		nextbtn = $items.next();
        		if((dltop =$dl.offset().top) >0||($dl.outerHeight(true)- dltop-top) >h){
        ...
        _refreshPageButton: function(panel){
            ...
            itemsH =$items.outerHeight(true),dlH = $dl.outerHeight(true),
        	...
        	if(dlH > itemsH){
        		if($up.is(":hidden")){
            		$items.outerHeight(itemsH - $up.outerHeight(true)*2);
            		$up.show();
            		$down.show();
        		}
        ...
        _getPageItems:function(panel, $dt){
        	var items = $(".om-itemselector-items", panel), itemsH = items.outerHeight(true),
        	$dl = $(">dl", items), pageItems =[], hasPageButton = items.next(":visible").length>0,
        	dtH = $dt.outerHeight(true), num = itemsH/dtH, dltop = $dl.offset().top,
...
;(function($){
    ...
    $.omWidget('om.omMenu', {
        ...
        show : function(triggerEle){
            ...
            if((left + element.outerWidth(true)) > document.documentElement.clientWidth){ //当右边距离过短的时候会将提示框调整到左边
                left = left - element.outerWidth(true) - 20;
            }
	...
	$.omWidget("om.omScrollbar" , {
			...
			$(bars).each(function(index , $bar){
				...
				$bar.omDraggable({
					...
					onDrag: function(ui, event){
						...
						type == 'v'?
							$elem.scrollTop( startPos + self._getInt( (p.top-op.top)*$elem.innerHeight()/$bar.outerHeight(true)) )
							:$elem.scrollLeft( startPos + self._getInt( (p.left-op.left)*$elem.innerWidth()/$bar.outerWidth(true)) );
					}
				}).hover(function(){
					$(bars).each(function(index , $bar){
						$bar._hover = true;
		...
		_mousewheelListener : function(event){
	        ...
	        var differ = delta / 120 * 10, 
	        	...
	        	distance = contentHeight - $bar.outerHeight(true);//滚动条最大滚动距离
	        ...
	        if(differ + barCurTop < 0){
	        	...
	        	$bar.css("top" , "+="+differ );
	        	$elem.scrollTop( $elem.scrollTop() + parseInt(differ * $elem.innerHeight()/$bar.outerHeight(true)) );
	        }
	        event.preventDefault();
		},
		...
		_resize : function($bar){
			var $elem = this.element,
				...
				w = $elem.width(),
				h = $elem.height(),
				ow = $elem.outerWidth(true),
				oh = $elem.outerHeight(true),
			...
			if('v' === type){
				...
				if(size > h){
					...
					$bar.css({
						"left" : offset.left + ow - $bar.outerWidth(true) - br,
				...
				if(size > $elem.innerWidth()){
					...
					$bar.css({
						...
						"top" : offset.top + oh - $bar.outerHeight(true) - bb
					});
				}else{
					$bar._enable = false;
				}
...
;(function($){
    ...
    $.omWidget('om.omSuggestion', {
        ...
        showMessage: function(message){
            ...
            dropList.parent().css('left', inputEl.offset().left).css('top',inputEl.offset().top+inputEl.outerHeight(true));
            var listWidth = this.options.listWidth;
            if (!listWidth) {//没有定义
                dropList.parent().width(inputEl.outerWidth(true));
        ...
        _scrollToAndSelect:function(all,index,dropList){
            ...
            if (targetTop <= 0) {
                ...
                //需要向下滚动滚动条
                var offset = targetTop + target.outerHeight(true) - dropList.height();
                if (offset > 0) {
        ...
        _buildDropList:function(records,text){
            ...
            if (all.length > 0) {
                dropList.parent().css('left', parseInt(inputEl.offset().left)).css('top',inputEl.offset().top+inputEl.outerHeight(true));
                var listWidth = this.options.listWidth;
                if (!listWidth) {//没有定义
                    dropList.parent().width(inputEl.outerWidth(true));
                } else if (listWidth !== 'auto') {
	...
	//            omtabs.children(':first').css('width',options.width);
		...
		_renderBody : function() {
	        ...
	        if (options.height !== 'auto') {
	            var omtabsHeight = $self.innerHeight(),
	                headersHeight = $self.find('>div.om-tabs-headers').outerHeight(true);
    	...
    	_close : function(index) {
	        ...
	        if ($panels.children().length == 0) {
	            $panels.css({height : omtabsHeight - $headers.outerHeight(true)});
	        }
        ...
        _adjustPosition : function(event){
           ...
           if(event && !options.region){
               ...
               if(options.region == 'right'){ //offset向右下偏 right作为默认配置
                   ...
                   top = top - self.element.height() - self.adjustHeight - options.offset[0];
                   self.tip.find('.tip-anchor').css({'top':options.anchorOffset+3,'left': self.tip.outerWidth(true)-2}); //将anchor向右移动tip的宽度
                   ...
                   self.tip.find('.tip-anchor').css({'top':self.tip.outerHeight(true)-3, 'left':options.anchorOffset+3}); //25+4
