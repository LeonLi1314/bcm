                ...
                if (!options.disabled && !options.readOnly && !options.multi) {
                    if (self.hasManualInput) {
                        ...
                        if (text !== '') {
                            ...
                            var index =allInputText?allInputText.indexOf(text):-1;
                            if (index > -1) {
...
(function($) {
    ...
    $.omWidget.addInitListener('om.omGrid',function(){
        ...
        this.tbody.delegate('tr.om-grid-row','dblclick',function(event){
        	...
        	if(!self.editfisrt){
        		if(ops.onAfterShowEdit){
        			//第一次编辑时回调
            		ops.onAfterShowEdit.call(self) ;
            	}
        		self.editfisrt=true;
        	}
        }); 
	 	...
	 	*/
	 	$.extend(this , {
	    	...
	    	doEdit:
				...
				if(!self._editView||!self._editView.editing){
					return true;
				}
